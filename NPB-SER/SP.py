...
def set_global_variables():
	...
	global u
	global us, vs, ws, qs, rho_i, speed, square
	global rhs, forcing
	global cv, rhon, rhos, rhoq, cuf, q
	global ue, buf
	global lhs, lhsp, lhsm
	...
	speed = numpy.zeros((KMAX, JMAXP+1, IMAXP+1), dtype=numpy.float64())
	...
	cv = numpy.zeros(npbparams.PROBLEM_SIZE, dtype=numpy.float64())
	rhon = numpy.zeros(npbparams.PROBLEM_SIZE, dtype=numpy.float64())
	rhos = numpy.zeros(npbparams.PROBLEM_SIZE, dtype=numpy.float64())
	rhoq = numpy.zeros(npbparams.PROBLEM_SIZE, dtype=numpy.float64())
	cuf = numpy.zeros(npbparams.PROBLEM_SIZE, dtype=numpy.float64())
	q = numpy.zeros(npbparams.PROBLEM_SIZE, dtype=numpy.float64())
	
	ue = numpy.zeros((5, npbparams.PROBLEM_SIZE), dtype=numpy.float64())
	buf = numpy.zeros((5, npbparams.PROBLEM_SIZE), dtype=numpy.float64())
	
	lhs = numpy.zeros((IMAXP+1, IMAXP+1, 5), dtype=numpy.float64())
	lhsp = numpy.zeros((IMAXP+1, IMAXP+1, 5), dtype=numpy.float64())
	lhsm = numpy.zeros((IMAXP+1, IMAXP+1, 5), dtype=numpy.float64())
...
# ---------------------------------------------------------------------
#
def verify(rhs, u, forcing):
	global rho_i, us, vs, ws, square, qs, speed 
	...
	# ---------------------------------------------------------------------
	error_norm(xce)
	compute_rhs(rhs, rho_i, us, vs, ws, square, qs, u, speed, forcing)
	rhs_norm(xcr)
	for m in range(5):
		xcr[m] = xcr[m] / dt
	...
	#Class S does not produce right results in Python (Not a Number - NAN error)
	if npbparams.CLASS == 'S':
		print("   *class S does not produce right results (NAN error). Please, use another class to validate them")
...
# ---------------------------------------------------------------------
@njit
def tzetar(rhs, us, vs, ws, speed, qs):
	#if timeron: Not supported by @njit
	#	c_timers.timer_start(T_TZETAR)
	for k in range(1, nz2+1):
		for j in range(1, ny2+1):
			for i in range(1, nx2+1):
				xvel = us[k][j][i]
...
# ---------------------------------------------------------------------
@njit
def z_solve(rhs, rho_i, cv, ws, rhos, lhs, lhsp, lhsm, speed, us, vs, qs):
	#if timeron: Not supported by @njit
	...
	for j in range(1, ny2+1):
		...
		# FORWARD ELIMINATION  
		# ---------------------------------------------------------------------
		for k in range(grid_points[2]-2):
			k1 = k+1
			k2 = k+2
			for i in range(1, nx2+1):
				fac1 = 1.0/lhs[k][i][2]
		...
		# ---------------------------------------------------------------------
		for k in range(grid_points[2]-3, -1, -1):
			k1 = k+1
			k2 = k+2
			for i in range(1, nx2+1):
				for m in range(3):
	...
	#END for j in range(1, ny2+1)
	#if timeron: Not supported by @njit
	#	c_timers.timer_stop(T_ZSOLVE)
	tzetar(rhs, us, vs, ws, speed, qs)
#END z_solve()
...
# ---------------------------------------------------------------------
@njit
def y_solve(rhs, rho_i, cv, vs, rhoq, lhs, lhsp, lhsm, speed):
	#if timeron: Not supported by @njit
	...
	for k in range(1, grid_points[2]-1):
		...
		# FORWARD ELIMINATION  
		# ---------------------------------------------------------------------
		for j in range(grid_points[1]-2): 
			j1 = j+1
			j2 = j+2
			for i in range(1, grid_points[0]-1):
				fac1 = 1.0/lhs[j][i][2]
		...
		# the first three factors
		# ---------------------------------------------------------------------
		for j in range(grid_points[1]-3, -1, -1):
			j1=j+1
			j2=j+2
			for i in range(1, grid_points[0]-1):
				for m in range(3):
...
# ---------------------------------------------------------------------
@njit
def x_solve(rhs, rho_i, cv, us, rhon, lhs, lhsp, lhsm, speed):
	#if timeron: Not supported by @njit
	...
	for k in range(1, nz2+1):
		...
		# ---------------------------------------------------------------------
		for j in range(1, ny2+1):
			for i in range(grid_points[0]-2):
				i1 = i+1
				i2 = i+2
				fac1 = 1.0/lhs[j][i][2]
				lhs[j][i][3] = fac1*lhs[j][i][3]
		...
		# ---------------------------------------------------------------------
		for j in range(1, ny2+1):
			for i in range(grid_points[0]-3, -1, -1):
				i1 = i+1
				i2 = i+2
				for m in range(3):
					rhs[k][j][i][m] = ( rhs[k][j][i][m]- 
...
# ---------------------------------------------------------------------
@njit
def txinvr(rhs, rho_i, us, vs, ws, speed, qs):
	#if timeron: Not supported by @njit
	#	c_timers.timer_start(T_TXINVR)
	for k in range(1, nz2+1):
		for j in range(1, ny2+1):
			for i in range(1, nx2+1):
				ru1 = rho_i[k][j][i]
				...
				rhs[k][j][i][4] = t2+t3
	#if timeron: Not supported by @njit
	#	c_timers.timer_stop(T_TXINVR)
#END txinvr()


@njit
def compute_rhs(rhs, rho_i, us, vs, ws, square, qs, u, speed, forcing): #Receive arrays as parameters
	#if timeron: Not supported by @njit
...
def adi(rhs, u, forcing,
		us, vs, ws, qs, rho_i, speed, square,
		cv, rhon, rhoq, rhos,
		lhs, lhsp, lhsm):
	if timeron:
		c_timers.timer_start(T_RHS)
	...
	if timeron:
		c_timers.timer_start(T_TXINVR)
	txinvr(rhs, rho_i, us, vs, ws, speed, qs)
	if timeron:
		c_timers.timer_stop(T_TXINVR)
	
	if timeron:
		c_timers.timer_start(T_XSOLVE)
	x_solve(rhs, rho_i, cv, us, rhon, lhs, lhsp, lhsm, speed)
	if timeron:
		c_timers.timer_stop(T_XSOLVE)
		
	if timeron:
		c_timers.timer_start(T_YSOLVE)
	y_solve(rhs, rho_i, cv, vs, rhoq, lhs, lhsp, lhsm, speed)
	if timeron:
		c_timers.timer_stop(T_YSOLVE)
		
	if timeron:
		c_timers.timer_start(T_ZSOLVE)
	z_solve(rhs, rho_i, cv, ws, rhos, lhs, lhsp, lhsm, speed, us, vs, qs)
	if timeron:
		c_timers.timer_stop(T_ZSOLVE)
...
def main():
	...
	print("\n\n NAS Parallel Benchmarks 4.1 Serial Python version - SP Benchmark\n")
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description='NPB-PYTHON-SER SP')
