...
df <- read_nvprof_trace("out_LU_B_cuda_2it.csv");
df$Name <-  ifelse(grepl("setbv_gpu_kernel_3", df$Name), "setbv_gpu_kernel_3", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_2", df$Name), "setbv_gpu_kernel_2", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_1", df$Name), "setbv_gpu_kernel_1", as.character(df$Name));
df$Name <-  ifelse(grepl("erhs_gpu_kernel_4", df$Name), "erhs_gpu_kernel_4", as.character(df$Name));
df$Name <-  ifelse(grepl("erhs_gpu_kernel_3", df$Name), "erhs_gpu_kernel_3", as.character(df$Name));
df$Name <-  ifelse(grepl("erhs_gpu_kernel_2", df$Name), "erhs_gpu_kernel_2", as.character(df$Name));
df$Name <-  ifelse(grepl("erhs_gpu_kernel_1", df$Name), "erhs_gpu_kernel_1", as.character(df$Name));
df$Name <-  ifelse(grepl("rhs_gpu_kernel_4", df$Name), "rhs_gpu_kernel_4", as.character(df$Name));
df$Name <-  ifelse(grepl("rhs_gpu_kernel_3", df$Name), "rhs_gpu_kernel_3", as.character(df$Name));
df$Name <-  ifelse(grepl("rhs_gpu_kernel_2", df$Name), "rhs_gpu_kernel_2", as.character(df$Name));
df$Name <-  ifelse(grepl("rhs_gpu_kernel_1", df$Name), "rhs_gpu_kernel_1", as.character(df$Name));
df$Name <-  ifelse(grepl("setiv_gpu_kernel", df$Name), "setiv_gpu_kernel", as.character(df$Name));
df$Name <-  ifelse(grepl("ssor_gpu_kernel_1", df$Name), "ssor_gpu_kernel_1", as.character(df$Name));
df$Name <-  ifelse(grepl("jacld_blts_gpu_kernel", df$Name), "jacld_blts_gpu_kernel", as.character(df$Name));
df$Name <-  ifelse(grepl("jacu_buts_gpu_kernel", df$Name), "jacu_buts_gpu_kernel", as.character(df$Name));
df$Name <-  ifelse(grepl("ssor_gpu_kernel_2", df$Name), "ssor_gpu_kernel_2", as.character(df$Name));
df$Name <-  ifelse(grepl("l2norm_gpu_kernel", df$Name), "l2norm_gpu_kernel", as.character(df$Name));
df$Name <-  ifelse(grepl("norm_gpu_kernel", df$Name), "norm_gpu_kernel", as.character(df$Name));
df$Origin <- "CUDA (class B)";
df$Kernel <- "LU";
df_traces <- rbindlist(list(df_traces, df));

df <- read_nvprof_trace("out_LU_B_numba_2it.csv");
df$Name <-  ifelse(grepl("setbv_gpu_kernel_3", df$Name), "setbv_gpu_kernel_3", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_2", df$Name), "setbv_gpu_kernel_2", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_1", df$Name), "setbv_gpu_kernel_1", as.character(df$Name));
df$Name <-  ifelse(grepl("erhs_gpu_kernel_4", df$Name), "erhs_gpu_kernel_4", as.character(df$Name));
...
df$Kernel <- "LU";
df$Duration <- df$Duration / 1e3 #Divide by 1000 to convert to uSec
df_traces <- rbindlist(list(df_traces, df));

df <- read_nvprof_trace("out_LU_C_cuda_2it.csv");
df$Name <-  ifelse(grepl("setbv_gpu_kernel_3", df$Name), "setbv_gpu_kernel_3", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_2", df$Name), "setbv_gpu_kernel_2", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_1", df$Name), "setbv_gpu_kernel_1", as.character(df$Name));
df$Name <-  ifelse(grepl("erhs_gpu_kernel_4", df$Name), "erhs_gpu_kernel_4", as.character(df$Name));
...
df <- read_nvprof_trace("out_LU_C_numba_2it.csv");
df$Name <-  ifelse(grepl("setbv_gpu_kernel_3", df$Name), "setbv_gpu_kernel_3", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_2", df$Name), "setbv_gpu_kernel_2", as.character(df$Name));
df$Name <-  ifelse(grepl("setbv_gpu_kernel_1", df$Name), "setbv_gpu_kernel_1", as.character(df$Name));
df$Name <-  ifelse(grepl("erhs_gpu_kernel_4", df$Name), "erhs_gpu_kernel_4", as.character(df$Name));
...
df_transfer$Origin <- factor(df_transfer$Origin, levels = c("CUDA (class B)", "Numba (class B)", "CUDA (class C)", "Numba (class C)"))
df_transfer$Name <- factor(df_transfer$Name, levels = c("rhs_gpu_kernel_4", "rhs_gpu_kernel_3", "rhs_gpu_kernel_2", "rhs_gpu_kernel_1", "ssor_gpu_kernel_2", "jacu_buts_gpu_kernel",  "jacld_blts_gpu_kernel",  "ssor_gpu_kernel_1"))
#df_transfer$Origin
#df_transfer$sum
...
gp1 <- df_transfer %>%
  ...
  #ylab("Time (s)")+
  labs(x=element_blank(), y="Time (s)", fill="LU: GPU kernels") +
  theme_bw()+
...
gp1
ggsave("lu_interation_time.pdf", plot=gp1, width=8, height=3.5);
```
