...
df <- read_nvprof_trace("out_CG_B_cuda_2it.csv");
df$Name <-  ifelse(grepl("gpu_kernel_one", df$Name), "gpu_kernel_one", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_two", df$Name), "gpu_kernel_two", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_three", df$Name), "gpu_kernel_three", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_four", df$Name), "gpu_kernel_four", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_five_1", df$Name), "gpu_kernel_five_1", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_five_2", df$Name), "gpu_kernel_five_2", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_six", df$Name), "gpu_kernel_six", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_seven", df$Name), "gpu_kernel_seven", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_eight", df$Name), "gpu_kernel_eight", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_nine", df$Name), "gpu_kernel_nine", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_ten_1", df$Name), "gpu_kernel_ten_1", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_ten_2", df$Name), "gpu_kernel_ten_2", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_eleven", df$Name), "gpu_kernel_eleven", as.character(df$Name));
df$Origin <- "CUDA (class B)";
df$Kernel <- "CG";
df_traces <- rbindlist(list(df_traces, df));

df <- read_nvprof_trace("out_CG_B_numba_2it.csv");
df$Name <-  ifelse(grepl("gpu_kernel_one", df$Name), "gpu_kernel_one", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_two", df$Name), "gpu_kernel_two", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_three", df$Name), "gpu_kernel_three", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_four", df$Name), "gpu_kernel_four", as.character(df$Name));
...
df <- read_nvprof_trace("out_CG_C_cuda_2it.csv");
df$Name <-  ifelse(grepl("gpu_kernel_one", df$Name), "gpu_kernel_one", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_two", df$Name), "gpu_kernel_two", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_three", df$Name), "gpu_kernel_three", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_four", df$Name), "gpu_kernel_four", as.character(df$Name));
...
df <- read_nvprof_trace("out_CG_C_numba_2it.csv");
df$Name <-  ifelse(grepl("gpu_kernel_one", df$Name), "gpu_kernel_one", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_two", df$Name), "gpu_kernel_two", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_three", df$Name), "gpu_kernel_three", as.character(df$Name));
df$Name <-  ifelse(grepl("gpu_kernel_four", df$Name), "gpu_kernel_four", as.character(df$Name));
...
df_transfer$Name <- factor(df_transfer$Name, levels = c("gpu_kernel_eleven", "gpu_kernel_ten_2", "gpu_kernel_ten_1", "gpu_kernel_nine", "gpu_kernel_eight", "gpu_kernel_seven", "gpu_kernel_six", "gpu_kernel_five_2", "gpu_kernel_five_1", "gpu_kernel_four", "gpu_kernel_three",  "gpu_kernel_two",  "gpu_kernel_one", "[CUDA memcpy DtoH]"))
#df_transfer$Origin
#df_transfer$sum
#df_transfer$Name
df_transfer[df_transfer$Name == "gpu_kernel_three"]$sum
df_transfer[df_transfer$Name == "gpu_kernel_four"]$sum
df_transfer[df_transfer$Name == "gpu_kernel_five_1"]$sum
df_transfer[df_transfer$Name == "gpu_kernel_five_2"]$sum
df_transfer[df_transfer$Name == "gpu_kernel_six"]$sum
df_transfer[df_transfer$Name == "gpu_kernel_seven"]$sum
df_transfer[df_transfer$Name == "gpu_kernel_eight"]$sum
...
gp1 <- df_transfer %>%
  ...
  labs(x=element_blank(), y="Time (s)", fill="CG: GPU kernels/transfers") +
  theme_bw()+
...
gp1
ggsave("cg_interation_time.pdf", plot=gp1, width=8, height=3.8);
```
